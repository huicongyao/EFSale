...
from metaheuristics.sbpso import SBPSO
...
class MEML_SBPSO:
    ...
    def __init__(self, num_tasks: int, gen: int, lr: float, ti: int, total_pop_size: int, f: Callable, weight: float,
        ...
        self.tasks = [SBPSO(task_param["dim"], task_param["dim_range"], task_param["num_obj"],
    ...
    def run(self, print_result: bool) -> Tuple[List, List]:
        ...
        for g in range(2, self.gen + 1):
            ...
            self.result.append(self.tasks[-1].gbest_score)
    ...
    def generational_result(self, g: int, print_result: bool) -> None:
        for idx, task in enumerate(self.tasks):
            ...
            if print_result:
                print("Task-{} ---> gen: {}, best: {}, mean: {}, pop size: {}".format(idx, g, task.gbest_score,
    ...
    def likelihood_main_pop_loov(pop):
        if len(pop) == 1:
            likelihood_vec = []
            prob_model = MEML_SBPSO.univarate_marginal_distribution(pop)
            likelihood_vec.extend(MEML_SBPSO.likelihood_from_umda(pop, prob_model.prob_vec))
        else:
            ...
            for i in range(k_folds):
                ...
                prob_model = MEML_SBPSO.univarate_marginal_distribution(model_pop)
                likelihood_vec.extend(MEML_SBPSO.likelihood_from_umda(batch_pop, prob_model.prob_vec))
            # prob_model = MEML_SBPSO.univarate_marginal_distribution([pop])
        return likelihood_vec
